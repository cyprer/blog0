...
4. 返回ans数组。
- 补一个图
<div style="text-align:center; border:1px solid #eee; padding:20px;">
  <img id="algoImg" src="https://your-valid-image-link.com/algorithm.jpg" 
       style="max-width:800px; border:1px solid #ddd;">
  <div style="margin-top:15px;">
    <button onclick="switchImg('prev')">← 上一张</button>
    <button onclick="switchImg('next')">下一张 →</button>
  </div>
</div>
...
function switchImg(action) {
  ...
  if(action === 'next' && current < imageList.length-1) current++;
